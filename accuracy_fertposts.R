...
fertsolutions<- read.csv("https://raw.githubusercontent.com/Jasper-Hewitt/elec_fertility/main/data/manual_fertsolutions.csv")
    ...
    body = list(
      ...
      messages = list(list(role = "system", content = "you are a researcher that does text classification in facebook posts related to 生育率. you only answer with numbers"),
                      list(role = "user", content = paste0("what kind of policy direction does the following post offer to solve the problem. 
                                                            label 1友善職場: examples include: 安胎假、產檢假, 產假、陪產假, 育嬰留職停薪, 家庭照顧假, 企業托育設施或措施.
                                                            
                                                            label 2經濟支持: examples include: 妊娠生產醫療給付,各類保險生育給付, 地方政府生育津貼, 地方政府低收入戶生, 育補助+
                                                            ...
                                                            label 3教保環境:examples include : 孕產婦關懷中心,產前健康檢查,公立幼兒園、非營利幼兒園,公私協力托嬰中心,托育資源中心,居家式托育服務,兒童預防保健服務(0-未滿 7 歲) ,收出養媒合及寄養安置,兒童三級預防措施，防治兒虐事件.
