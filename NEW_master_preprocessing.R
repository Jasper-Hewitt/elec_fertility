...
master_df$Campaign_duration <- floor(as.numeric(difftime(as.Date("2022-12-27"), master_df$Date_Started_Running, units = "days")))
...
#plot in a flipped barplot (consider also adding the chinese name)
ggplot(by_candidate, aes(x = reorder(Candidate, posts), y = posts)) +  # arrange from highest total to lowest 
...
#average daily posts (divide number of fertility posts by campaign duration by Campaign_duration!!) then just plot!

#___________#__________### 4: NUMBER OF POSTS OVERTIME (OF TOP 5 CANDIDATES)? ####___________#__________#



#___________#__________### 4: fertility relative to other topics (fix master to master_df!) ####___________#__________# 
